...
ADMIN_ID = 806288103  # Your admin ID
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("By https://t.me/flashmainchannel 🚀Press the Attack button to start CHIN TAPAK DUM DUM (●'◡'●)", reply_markup=reply_markup)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.callback_query.data == 'attack':
        await update.callback_query.message.reply_text("By https://t.me/FLASH_502 Please enter the target, port, and time in the format:<target> <port> <time>🚀🚀")
...
async def start_attack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.callback_query.message.reply_text(f"CHIN TAPAK DUM DUM (●'◡'●) FeedBack De Dio Yaad se 😡 :- 👉 https://t.me/FLASH_502 {target_ip}:{target_port} for {attack_time} seconds https://t.me/addlist/aGuzDCoYQChkN2Q1")
...
async def stop_attack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not process or process.poll() is not None:
        await update.callback_query.message.reply_text("CHIN TAPAK DUM DUM NHI CHAL RHA (●'◡'●) t.me/FLASHddosFEEDBACK ")
        return
...
async def reset_attack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    target_port = None
    attack_time = None
    await update.callback_query.message.reply_text("Attack reset. By https://t.me/FLASH_502 Please enter the target, port, and time in the format:<target> <port> <time>🚀")
